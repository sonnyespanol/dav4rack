    ...
    def call(env)
      begin
        ...
        begin
          ...
          pc("Request Method", request.request_method)
          pc("Request Body", request.body.read)
          request.body.rewind #Removing this line would cause a bug that prevents request from being read

          res = controller.send(request.request_method.downcase)

          pc("Response Body", response.body)
